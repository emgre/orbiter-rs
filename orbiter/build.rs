...
fn main() -> Result<(), std::io::Error> {
    ...
    // List of C++ files
    let header_files = ["src/cpp/types.h"];
    let cpp_files = [
        "src/cpp/main.cpp",
        "src/cpp/module.cpp",
        "src/cpp/object.cpp",
    ...
    println!(
        ...
        lib_path.join("orbiter.lib").to_string_lossy()
    );
    println!(
        "cargo:rerun-if-changed={}",
        lib_path.join("Orbitersdk.lib").to_string_lossy()
    );
    println!(
        "cargo:rerun-if-changed={}",
        include_path.join("Orbitersdk.h").to_string_lossy()
    );
    ...
    let is_static_crt =
        env::var("CARGO_CFG_TARGET_FEATURE").map_or(false, |val| val.contains("crt-static"));
    if !is_static_crt {
        ...
        println!(
            "cargo:warning=To avoid this, add the following lines to you .cargo/config.toml file:"
        );
        ...
        println!(
            "cargo:warning=(you might need a `cargo clean` to take the modifications into account)"
        );
    }
    ...
    if is_static_crt {
        cc.static_crt(true).ar_flag("/NODEFAULTLIB:msvcrt.lib");
    }
