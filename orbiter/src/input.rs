...
/// Mouse event.
///
/// The mouse flags can be used to check "virtual keys". For example, it can
// tell if the CTRL key was pressed when a mouse button was clicked.
///
/// The `x` and `y` coordinates are relative to the upper-left corner of the
/// window.
#[derive(Debug, Copy, Clone)]
pub struct MouseEvent {
    /// Mouse event type.
    pub event_type: MouseEventType,
    /// Associated "virtual keys" of the event.
    pub flags: MouseFlags,
    /// X coordinate of the mouse position, relative to the upper-left corner of the window.
    pub x: u32,
    /// Y coordinate of the mouse position, relative to the upper-left corner of the window.
    pub y: u32,
}

impl MouseEvent {
    pub(crate) fn from(event: UINT, state: DWORD, x: DWORD, y: DWORD) -> Self {
        Self {
...
///
/// Button events have an enum to describe what happened to the button.
///
/// Mouse wheel events have a `i16` value that represents the distance the wheel
/// rotated. These values are expressed in multiples or divisions of 120. A positive
...
pub enum MouseEventType {
    /// The cursor moved.
    Move,
    /// The left button was clicked, released or double-clicked.
    LeftButton(MouseButtonEventType),
    /// The right button was clicked, released or double-clicked.
    RightButton(MouseButtonEventType),
    /// The middle button was clicked, released or double-clicked.
    MiddleButton(MouseButtonEventType),
    /// The first X button was clicked, released or double-clicked.
    XButton1(MouseButtonEventType),
    /// The second X button was clicked, released or double-clicked.
    XButton2(MouseButtonEventType),
    /// The mouse scroll wheel was rotated.
    Wheel(i16),
    /// The mouse horizontal scroll wheel was tilted or rotated.
    HorizontalWheel(i16),
    /// An unknown mouse event occured.
    Unknown(u32),
}

/// Type of mouse button events.
#[derive(Debug, Copy, Clone, PartialOrd, PartialEq)]
pub enum MouseButtonEventType {
    /// The user pressed the button down.
    Down,
    /// The user released the button.
    Up,
    /// The user double-clicked the button.
    DoubleClick,
}

impl MouseEventType {
    fn from(event: UINT, wparam: DWORD) -> Self {
        match event {
...
bitflags! {
    /// Bitflags of virtual keys that are pressed down during a mouse event.
    pub struct MouseFlags: u16 {
        /// The left mouse button is down.
        const LEFT_BUTTON = winuser::MK_LBUTTON as u16;
        /// The right mouse button is down.
        const RIGHT_BUTTON = winuser::MK_RBUTTON as u16;
        /// The SHIFT key is down.
        const SHIFT = winuser::MK_SHIFT as u16;
        /// The CTRL key is down.
        const CONTROL = winuser::MK_CONTROL as u16;
        /// The middle mouse button is down.
        const MIDDLE_BUTTON = winuser::MK_MBUTTON as u16;
        /// The first X button is down.
        const X_BUTTON_1 = winuser::MK_XBUTTON1 as u16;
        /// The second X button is down.
        const X_BUTTON_2 = winuser::MK_XBUTTON2 as u16;
    }
}

fn get_mouse_flags(wparam: DWORD) -> MouseFlags {
    unsafe { MouseFlags::from_bits_unchecked(winuser::GET_KEYSTATE_WPARAM(wparam as WPARAM)) }
}

fn get_x_button_event(button_event: MouseButtonEventType, wparam: DWORD) -> MouseEventType {
    match winuser::GET_XBUTTON_WPARAM(wparam as WPARAM) {
        winuser::XBUTTON1 => MouseEventType::XButton1(button_event),
        winuser::XBUTTON2 => MouseEventType::XButton2(button_event),
        _ => MouseEventType::Unknown(wparam),
    }
}

fn get_wheel_delta(wparam: DWORD) -> i16 {
    winuser::GET_WHEEL_DELTA_WPARAM(wparam as WPARAM)
}
